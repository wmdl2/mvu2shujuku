...
已经通过的检查不机械重复。只有新增相关源码改动、检查失败或仍有具体疑点时才补跑相应范围；实机失败先定位并只重跑受影响场景。仅修正测试脚本的宿主注册方式、补文档或更新交接时，不因此重跑已通过且源码未变的全部回归。全量回归不是重新执行已完成的历史审查。

实机初次启动成本较高；同一批次需要连续定位时可在隔离环境使用 `--keep-server`，后续 `--reuse-server --only=...`，浏览器仍由测试脚本管理，结束后关闭本轮保留的服务器。不得复用用户日常酒馆，也不为单个问题重跑历史/重生成/UI 的整套场景。
...
本地批次记录沿用 `.tools/<批次>/baseline.json` 与 `verification.json`：命令、结果、runId、受测指纹、未覆盖项和服务清理状态。长日志保存在文件，只提取失败摘要和结果；等待测试期间不以无关阅读填充。减少重复操作不等于已测得额度节省，不能据此声称具体比例。
