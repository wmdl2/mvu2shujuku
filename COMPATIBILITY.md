...
> 适用于 MVU转数据库 v0.4.0。本文用于说明哪些 MVU 内容可以转换、哪些运行时行为由兼容层接管，以及当前无法等价模拟的边界。

v0.3.15 生成的桥支持扩展晚加载接管：停止旧任务、取消尚未提交的写入并返回 false，等待已发出的数据库调用结束。此前转换的卡需要重新转换以获得此能力；接管不会回滚已经发生的写入。

v0.3.18 新桥仅登记卡元数据，不携带第二套运行时。扩展未加载时自动重试并显示等待提示，登记成功后停止重试；转换卡需要本扩展运行。旧 v0.3.15–0.3.17 桥仍按退出协议交接。

本轮回归覆盖 JSON 文件导入、数组类型与原子提交、失败写入停止、异步切聊天和删楼取消、混合业务脚本保留及保存/下载结果一致性。旧数组布局缺少类型信息时仍按文本读取。SP·数据库核对版本为 9.2.5（`5c53f795832b194ec4baa74135e8ebd950122438`）；实机环境、场景与边界见[验收记录](docs/validation/2026-09-11-host.md)。
...
顶层有限数字使用 REAL 数值列与原值默认值，允许小数；运行时仍投影为原路径上的数字。数值表写入拒绝对象、字符串、布尔值和非有限数字；SQLite CHECK 校验数值存储类型，不自动把自然语言规则翻译成范围 CHECK。其他顶层标量与动态 JSON 组保持 JSON 表语义，旧聊天不会自动迁移类型。
...
- 初始化数据中的动态键名
- 初始化宏替换后的键名冲突检测
- 官方对象 `$meta`：`extensible`、`recursiveExtensible`、`required`、`template`
...
- 单例对象 → 单例表
- 动态字典 → 行表
- 动态字典索引签名中的业务名称（如 `[物品名称: string]`）→ 行表键列名；无声明时回退为“键名”
- 标量字典 → 键值行表
- 混合类型标量字典 → JSON 标量键值行表（保留字符串、数字、布尔与 `null`）
- null 样本或 Zod nullable/optional/nullish 声明的普通叶子 → 可缺失 JSON 标量列，区分 null、空字符串、0、false 与缺失键。字段级可空对象/数组整体编码；有 null 样本的 VWD 标量恢复值与静态描述。覆盖单例、动态行、嵌套关系行，详见[数据约定](docs/data-model.md#可空普通列)。
- 明确声明 nullable/optional/nullish 的顶层对象、数组、字典 → 单例完整 JSON 列，区分整组缺失、null、空容器和有值；模型与脚本使用同一表示，无需状态列。固定内部路径的数值范围、小枚举可生成 SQLite CHECK，动态元素约束保留说明并报告限制。明确声明可空的动态记录值也逐条保留为完整 JSON 列，支持多层关联，见[可空记录范围与验收](docs/validation/2026-09-21-nullable-records.md)。未声明容器可选完整 JSON 模式以保留空状态和嵌套 pair；原拆分布局及旧聊天不自动迁移，见[完整 JSON 容器](docs/validation/2026-09-22-full-json-containers.md)；顶层容器证据见[整组空值](docs/validation/2026-09-16-container-presence.md)。VWD 动态说明范围见下方专节。
...
- 表名、列名拼音转换与重名消歧
- 表名中的宏静态规范化（`{{user}}` → `user`、`{{getvar::阵营}}` → `getvar_阵营`）；不保留结构名的运行时求值语义
...
- 模板键展开后，note 按实际列名逐列保留描述与规则；相同约束不跨列折叠，DDL 也逐列生成
...
SQLite/双模式模板为可写 JSON 列提供按路径更新说明与局部替换示例；双模式的 SQL 示例仅适用于 SQL 填表，native 仍遵循宿主原生格式；
native 保留完整单元格写回。明确固定 Zod 附属对象的字段并回父表，保留原嵌套路径、字段约束、
只读与隐藏语义；动态字典不据当前大小合并，已有聊天不自动迁移。
...
开局事件链若交回 MVU/VWD 旧式 `[当前值, 描述]` 叶子，转换器仅对 layout 中明确的 `pair` / `jsonPairOptional` 列在 SQL 建表边界取当前值。普通数组、对象和 JSON 列不会被该兼容规则拆包。

**写入侧同样只落当前值（v0.3.18）**：脚本写回整段 `[当前值, 说明]` 时，写库边界按 layout 登记的 pair 路径取出当前值，数字 pair 保留数值、布尔 pair 写物理值 `0` / `1`、可空 JSON 标量列保留 `null` / 缺失的 JSON 编码。此前整段数组会进入单元格，使 SP 的 SyncBridge 在 SQLite hydrate 阶段报 `val.replace is not a function` 并回退原生；修复后 native 与 SQLite 都能正常写入、刷新后读回。该修复与 VWD 实验开关无关，默认转换即生效。

### VWD 动态说明（新转换、限定范围）

转换界面在 EJS 上下文桥就绪时自动请求动态说明能力；目标 SP 须支持隐藏内部列（9.2.5+）。无此能力时保留静态转换。Node 核心需显式传入 `{ vwdDescriptions: true }`；原 `vwdExperimental` 开关保留为旧静态实验测试入口，仍默认关闭，不代表新渲染路径未启用。

**支持：** 单例表中 layout 已登记为 `pair` / `jsonPairOptional`、模型可见的字段，以及新动态说明布局中带 pair 标记的数字/布尔字段（物理列类型不变，读回恢复 `[值, 当前说明]`）。每表一列隐藏说明覆盖数据；值和说明同批保存，实际填表时动态读取，note 本身不随更新改写。支持相同初始说明的字段分别更新、初始空说明事后补充、显式空串、刷新和删楼回退。普通说明保持原文；可能触发模板解析的说明以标注过的 JSON 字符串表示，存储和 MVU 读回仍是原始文本。列名、原业务规则和 SQL 示例保留。

**边界：** 需要保留生成的动态入口和所有字段插槽，并保持提示词模板扩展启用。缺少渲染能力、元数据列或匹配的模板时，说明变化在真实写入前整笔拒绝。在转换器中编辑模板时，保留入口/插槽可继续添加规则；删除这些结构会产生转换警告。普通拆分布局的动态行表、数组元素、对象值 VWD、任意历史批量填表上下文及旧卡自动迁移不在本节范围；完整 JSON 模式直接在当前数据里保存这些嵌套 pair 的值和说明，不使用隐藏覆盖列。数字/布尔扩展见[后续验收](docs/validation/2026-09-22-compatibility-followup.md)。依赖扩展被停用后，宿主仍可能跳过模板处理，不能宣称所有填表请求都有全局拦截保证。见[验收与成本](docs/validation/2026-09-16-vwd-prompt.md)。
...
- MVU/TH `{{format_message_variable::stat_data.…}}` 在世界书正文和表格 `note/check` 中均会改为运行时数据库读取；标量直出，对象/数组用 YAML，以 `$` 开头的键不展示。
...
`getMvuVariable/setMvuVariable` 支持 Lodash 风格的点号、数字下标与引号键路径；getter 不拆普通两元素数值/布尔数组。`setMvuVariable(..., { is_recursive: true })` 等待兼容总线异步监听器完成，调用方随后提交时可包含监听器修正。公共 setter 保留输入类型，不提前转换数字字符串或 Date；数据库写入仍受列类型约束。

**部分等价项：**

- `reloadInitVar` 从转换后的初始数据库模板恢复。
- `isDuringExtraAnalysis()` 当前固定返回 `false`。
- `getMvuData(options)` 返回当前数据库视图，不承诺历史 message_id 或 iframe 所在楼层查询。
- 普通 `replaceMvuData` 绑定调用时的最新 AI 回复；无法证明本楼可安全追加时，正式提交最终快照。已有可靠帧的当前回复先在内存完成批次规划：单次 CRUD 保留差量，多步混合更新合为一次导入，提交被拒绝不拆分补写。删楼、编辑、换 swipe 或新 AI 回复出现会取消旧任务。历史 `message_id` 定点写入仍不承诺；多作用域或旧格式可能持续使用快照，见[楼层归属修复](docs/validation/2026-09-16-write-floor.md)及[混合批次范围与失败边界](docs/validation/2026-09-16-atomic-batch.md)。
...
- SP 已持久化业务提交可依据 V2 日志来源等待结束监听并保存修正；合并窗口内去重，回放、自身提交与来源不明的通知不自动重算。旧楼业务修正先完成内存规划，只允许一次 CRUD；多步修正在真实写入前整笔拒绝，具体见[入口契约](docs/runtime.md#更新入口与提交契约)
- 每轮解析/业务事件从完整当前状态建立 display，delta 仅含本轮变化；这两个视图不独立持久化，普通读取 delta 为空
- 早期兼容 eventOn 支持停止函数与 eventOff，移除同一 handler 的重复注册
- 初始化事件的 `swipe_id`
- 避免同一总线重复广播
...
开局、显示与业务正则即使包含 `<UpdateVariable>` 或兼容 API，也不会仅据此删除；不要求有交互控件。只有整个替换体为纯旧变量快照宏时删除，隐藏/折叠清理规则仍保留。保留前端的 `format_message_variable` 宏改用数据库展示入口。脚本内的 `/<%|%>/` 标签检测正则转换为十六进制字符写法，避免被 EJS 当作模板。扩展还通过提示词模板的 `prompt_template_prepare` / `activateRegex` HTML 阶段保护运行时新增的同类前端；此运行时保护需要扩展和支持该接口的提示词模板插件，已按 1.17.9 源码核对，尚未实机联调。
...
- `VARIABLE_UPDATE_ENDED` 驱动刷新；含明确数据读取信号的事件型内联 HTML 保持原样并归为 event
- SP 自动填表回调转发为 `VARIABLE_UPDATE_ENDED`（含晚加载和聊天切换后重注册）
- 删除楼层、切换 swipe、发送/接收/编辑消息和生成收尾引发的聊天历史变化会校验事件前后完整表快照；即使 SP 版本未在回放路径调用外部回调，也会在运行时稳定后补发。普通 CRUD、SQL/自动填表仍由 SP 通用表更新回调覆盖
- 消息更新块以整表提交落到最新 AI 回复；删楼取消旧合并窗口，来源消息被编辑或切换后旧异步任务失效。同内容的新回复仍执行，已有回复移动楼层不重复执行
- 酒馆助手默认 `deleteChatMessages(..., {refresh: 'affected'})` 不发送 `MESSAGE_DELETED`；静默改动聊天需通过宿主事件或完整重载触发数据库回放
- 每次确认的表变化同时广播 `VARIABLE_UPDATE_ENDED` 与 `shujuku-table-updated`，并统一调用安全内联重读入口或精确刷新控件；批量写的回调抑制解除后会重试最后快照，不再永久丢通知
...
- 显式更新标记不再单独触发删除；业务 EJS 与非专名的剧情/更新块混合条目保留，规则文档解析失败也保留并报告核对。明确专名的无 EJS 输出文档、纯快照条目仍删除；自由文本无法保证完全自动分类，需复核报告
...
- 新行插入
- 行删除
- 脚本明确删除或重命名父行时按完整祖先键清理后代行；省略父组和历史孤儿不自动清理，SP 直接表编辑没有新增物理外键
- 数组按位置更新、尾部追加和尾删；多个数组操作合并为一次 `importTableAsJson`
- JSON 对象整列写回
- 动态字段写入 `_扩展数据`
- 写后立即读一致
- 首楼替换/删除后的开局连续性保护
...
多个数组操作需要宿主提供 `importTableAsJson`；缺少能力或规划快照已变化时在数组提交前失败。批量导入失败不自动回写旧全库快照，以免覆盖已经持久化的新数据；重试应基于实际快照。混合标量 CRUD 仍可能部分成功，这不是整个写批次的事务保证。
...
- 表格设置只修改本次转换模板；多选批量设置与逐表设置共用同一写入路径，下载/保存时刷新卡内模板和桥。已有聊天模板不自动变更。
- 世界书开关写 `exportConfig.injectIntoWorldbook`，与 SP 同名选项一致；不删除表、不停用自动填表，也不自动关闭 `extraIndexEnabled` 或改写原卡世界书中的显式读取。
- 转换配置新增可选的布尔注入字段，旧配置没有该字段时不覆盖新模板原值，不复用旧配置中的表结构。
- 旧转换卡的布局由卡内元数据决定，新扩展按已支持的旧布局分支读写，不自动换成新结构。新扩展继续兼容已支持的旧布局；桥的版本号不是自动迁移机制。
- 要使用新的表格布局或新编码能力，需重新转换原卡并使用对应模板建立新聊天。未知旧布局或已污染的历史单元格不自动修复。
