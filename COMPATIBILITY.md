...
> 适用于 MVU转数据库 v0.3.17。本文用于说明哪些 MVU 内容可以转换、哪些运行时行为由兼容层接管，以及当前无法等价模拟的边界。

v0.3.15 生成的桥支持扩展晚加载接管：停止旧任务、取消尚未提交的写入并返回 false，等待已发出的数据库调用结束。此前转换的卡需要重新转换以获得此能力；接管不会回滚已经发生的写入。

本轮新增回归覆盖 JSON 文件导入、顶层数组各 JSON 类型、失败写入停止、异步切聊天取消、混合业务脚本保留及保存/下载结果一致性。旧数组布局缺少类型信息时仍按文本读取。SP·数据库 9.2.4 已做相关源码接口核对，尚未完成该版本真实酒馆中的全部场景验收。
