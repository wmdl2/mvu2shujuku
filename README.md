...
转换器只会整条删除可确认属于 `[InitVar]`、已迁移的专用变量规则或纯 MVU 变量输出管线的世界书条目；没有 `[mvu_update]` 前缀的“变量更新规则”“变量处理指令集”等专名也会结合正文结构识别。若条目同时包含剧情、EJS 和更新块，会完整保留；其中 `stat_data` 的 `getMessageVar/setMessageVar` 调用由数据库兼容函数接管，其他消息变量仍按酒馆助手原语义运行。删除只发生在转换副本，原卡保持不变；旧管线不会用 `if (false)` 包裹，因为其中的 EJS 仍会被编译并可能产生重复声明或语法错误。
...
1. 打开扩展设置面板，选择输入：当前角色卡，或文件选择器（支持 `.json` / `.png`）。
   JSON 文件支持 UTF-8 BOM；自动导出格式跟随原文件类型。世界书为空但问候语含有效 `<initvar>` 的卡也可以转换。
2. 点击转换：
   ...
   - 或下载 角色卡 + 表格模板 + 转换报告。另有“数据桥源码（仅供调试）” `.js` 备份；数据桥已内嵌到转换后的角色卡，无需导入，该 `.js` 也不是酒馆助手的 `.json` 导入包。
...
应用配置、编辑模板参数或合并表格后，转换器会复用已完成的角色卡分析来更新下载和保存内容。没有改动时直接复用产物；改变转换规则时重新分析原卡。

新卡无需增加操作：下载转换后的角色卡/表格模板，或保存到 SillyTavern 时，扩展会以原角色卡名自动保存转换配置。只有已存在同名配置时才需选择替换或重命名。重新转换原卡或更新版卡时，可在「转换配置」下拉中主动选择旧配置。

外部表只记录来源（当前聊天/全局/指定全局预设）和表身份，不复制完整表。再次应用时会从来源读取最新版；来源被删除、表消失或表名冲突时会跳过并在摘要中说明。当前 SP·数据库公开 API 没有提供内部默认模板的只读导出，因此不把它列为可选来源。
...
- 解析教程式酒馆助手 Zod Schema：从 `registerMvuSchema(...)` 静态提取 `object/record/array`、基础类型、`enum/describe/prefault/default` 及标准 `_.clamp` 范围；不执行角色卡 JavaScript，无法等价迁移的自定义校验会写入转换报告。
- 顶层键 → 组；按结构推导表种类：组自身有直接标量字段=单例表、条目字典=行表、固定嵌套对象=用 `_` 递归展开、
  ...
  `${A|B}`（兼容 `${A/B}`）模板键下的规则会逐项展开，包括 `${A|B}经验` 这种带前后缀的合并字段；固定容器的整体 `check` 会传递到真正承载数据的子表。
...
- `[MvuUpdate]` 中的数值 `range` → `CHECK`、可选值数组 `range`/枚举 → `CHECK IN`、`format`/`check` → note、`_强制更新提醒` → “每次回复必须维护”。所有范围/枚举/格式均来自卡片自身规则。
