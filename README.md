...
结果页先展开「需人工处理」和「注意事项」，普通说明、自动转换记录和完整报告可按需展开；报告内容仍可完整复制、下载。高级兼容选项和合并其他模板折叠显示。

「表格设置」支持按表名筛选、勾选多张表，再把同一个更新参数数值应用到所选表；也可逐表修改或应用到全部。全选只选择当前列表，筛选不会清除已有选择；「应用到全部」包含筛选隐藏的表。
