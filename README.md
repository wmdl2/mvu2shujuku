...
**v0.4.0** 的变更见 [更新日志](CHANGELOG.md)；详细支持范围、运行时兼容接口与已知边界见 [MVU 转换与兼容清单](COMPATIBILITY.md)。
...
升级扩展后，已有转换卡继续按原布局运行，旧聊天不会自动改表或迁移。界面改进直接生效；要使用新的表格布局、完整 JSON 容器等转换功能时，再重新转换原卡并新建聊天。
...
> 推荐在 SP·数据库中使用 **SQLite 模式**，可利用字段约束和 JSON 列的局部更新。
> 模板统一按双模式生成，包含 DDL、业务规则和按模式区分的 JSON 更新说明。AI 填表时实际输出
> `insertRow/updateRow/deleteRow` DSL 还是 SQL，由 SP·数据库 插件自身的填表模式设置决定，
> 转换器不需要也不提供单独的 native/sqlite 选择。

JSON 列在 SQLite 模式下可按路径更新，减少重复输出未修改的数据；native 继续支持按单元格填写。
明确由 Zod 声明为固定结构的附属对象会并回父表列，动态记录和数组仍按各自结构存储。
明确声明可空的动态记录用身份列加完整 JSON 内容列保存，可区分 `null`、空容器与删除记录，无需状态列。
这些变化在重新转换时生效，已有聊天不会自动合并表格或迁移布局。

### 查看报告与设置表格

结果页先展开「兼容性待确认」和「注意事项」：这些是转换成功后的兼容提醒，部分行为可能与原卡不同，需要按使用情况核对。普通说明、自动转换记录和完整报告可按需展开；报告内容仍可完整复制、下载。高级兼容选项和合并其他模板折叠显示。

「表格设置」支持按表名筛选、勾选多张表，再把同一个更新参数数值应用到所选表，也可逐表修改。全选和「反选当前列表」只影响当前可见行；筛选不会清除已有选择，「应用到所选」仍包含隐藏的已选表。要批量设置全部表，清空筛选后全选，再应用到所选即可。

每张表都有「注入世界书」开关，也可对所选表批量开启/关闭。它对应 SP·数据库的「注入到世界书条目」：关闭不删除数据，不影响前端/脚本读写，也不会停用自动填表。额外索引和原卡世界书中显式读取数据的内容需单独检查，不能把它当作禁止所有模型读取的开关。

这些设置随下载/保存写入新模板，并记录在转换配置里，不自动修改已有聊天；旧配置缺少世界书注入字段时，沿用新模板本来的设置。
...
布局保存在转换卡内，主要读写逻辑由扩展提供，桥中的版本号不是自动迁移机制。升级扩展不会把旧聊天改成新布局；已识别的旧布局按原结构处理。只有需要使用新布局功能时，才需重新转换原卡并新建聊天。
...
- 对未声明可空的容器，可在转换前勾选「将容器保存为完整 JSON」：每个顶层对象/数组一列，相关子表合并，准确保留缺失、null、空容器和嵌套说明。推荐 SQLite 按路径局部更新；native 需更新整个 JSON 单元格。现有卡不自动迁移，见[使用与边界](docs/validation/2026-09-22-full-json-containers.md)。
- 对明确声明可空或可缺失的顶层对象、数组、字典，用完整 JSON 列区分“不存在、null、空容器”，无需额外状态列；SQL 支持路径更新，native 更新完整单元格。旧卡需重新转换，见[支持范围](docs/validation/2026-09-16-container-presence.md)。
- 新转换支持单例表中已识别的文本/可空及数字/布尔 pair 动态说明：转换界面在提示词模板扩展 EJS 桥就绪、SP 版本支持隐藏列时自动接入。说明随数据保存，在实际填表请求时读取；支持补充初始空说明、清空、刷新和删楼回退。含模板标记的说明以可逆 JSON 字符串显示，避免再次执行。需要保持提示词模板扩展启用；旧转换卡需重新转换，范围和证据见[动态说明验收](docs/validation/2026-09-16-vwd-prompt.md)。
...
v0.3.16 支持 `variables_update_rules` 英文规则外壳，并将 TypeScript 纯字符串联合（如 `'高' | '中' | '低'`）按字段路径迁入表格提示与枚举 CHECK。含交互控件的开局面板会保留。正文前端若用 `/<%|%>/` 检测标签，转换器和扩展渲染阶段会保护该写法，避免 EJS 的 `Unexpected token '|'` 报错。已转换卡需重新转换以更新表格规则；动态前端保护需更新并加载扩展。
